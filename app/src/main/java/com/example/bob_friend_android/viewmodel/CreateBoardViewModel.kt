...
class CreateBoardViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun CreateBoard(boardTitle : String, boardContent: String, context: Context) {

        val userName = App.prefs.getString("username", "")
        val board = Board(boardTitle=boardTitle, boardContent=boardContent,userName=userName)
