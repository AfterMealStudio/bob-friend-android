...
import com.example.bob_friend_android.data.entity.BoardList
...
class ListViewModel: BaseViewModel() {
    ...
    fun setList(listPage: Int, type: String? = null, address: String? = null){
        ...
        ApiModule.apiBob.getRecruitments(listPage, type, address).enqueue(object : Callback<BoardList> {
    ...
    fun searchList(listPage: Int, category: String = "all", keyword: String, start: String? = null, end: String? = null){
        ...
        if (start != null && end != null) {
            ApiModule.apiBob.searchRecruitmentsTimeLimit(listPage, category, keyword, start, end).enqueue(object : Callback<BoardList> {
                override fun onResponse(call: Call<BoardList>, response: Response<BoardList>) {
                    Log.d(TAG, "setList : ${response.body()}")
            ...
            ApiModule.apiBob.searchRecruitments(listPage, category, keyword).enqueue(object : Callback<BoardList> {
                override fun onResponse(call: Call<BoardList>, response: Response<BoardList>) {
                    Log.d(TAG, "setList : ${response.body()}")
    ...
    fun setMarkers(zoom: Int, longitude: Double, latitude:Double) {
        _progressVisible.postValue(true)
        ApiModule.apiBob.getRecruitmentLocations(zoom, longitude, latitude).enqueue(object : Callback<LocationList> {
